...
class DefaultEventDispatcher implements EventDispatcher {
	...
	public function addEventListener($type, $listener, $priority = 0) {
		...
		// Check if the listener is callable
		if (!is_callable($listener)) {
			throw new InvalidArgumentException('Listener must be callable');
		}
		
		// Create queue for event type if it's the first listener for this type
	...
	public function removeEventListener($type, $listener) {
		...
		// Return if no listener for the given type is registered
